...
const TutorialsScreen = () => {
  ...
  const handleSort = (field: "createdAt" | "title", order: "asc" | "desc") => {
  ...
  const handleCardPress = (item: TUTORIALS_PROPS) => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    router.push({
  ...
  const renderTutorialCard = ({ item }: { item: TUTORIALS_PROPS }) => (
    <Pressable
    ...
    >
      ...
      <View className="p-3">
        <Text
          className="text-sm font-bold text-gray-900 mb-1 leading-5"
          numberOfLines={2}
        >
          {item.title}
        </Text>
        ...
        <Text
          className="text-gray-500 text-xs leading-4 mb-2"
          numberOfLines={2}
        >
          {item.description}
        </Text>

        {/* Watch Button - Visual Only */}
        <View className="flex-row items-center justify-center bg-[#6566fc]/10 px-3 py-2 rounded-lg mt-1">
          <MaterialCommunityIcons
            ...
            color="#6566fc"
          />
          <Text className="text-[#6566fc] font-semibold text-xs ml-1">
            Watch Now
          </Text>
        </View>
      </View>
