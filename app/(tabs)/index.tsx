...
const Index = () => {
  ...
  // Filter sessions locally by search (since backend doesn't have search)
  ...
  const applyFilters = (sessionType: string, mode: string) => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
  ...
  const activeFiltersCount =
    (selectedSessionType ? 1 : 0) + (selectedMode ? 1 : 0);
  ...
  const renderSessionCard = ({ item }: { item: any }) => (
    ...
    >
      ...
      <View className="p-4">
        {/* Title */}
        <Text
          className="text-xl font-bold text-gray-800 mb-3 leading-6"
          numberOfLines={2}
        >
          {item.title}
        </Text>
